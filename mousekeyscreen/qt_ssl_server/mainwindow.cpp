...
void MainWindow::initConnects()
{
    ...
    connect(m_server, SIGNAL(mouse_pos(QString , QString , QString)), this, SLOT(mouse_control(QString, QString, QString)));
    connect(m_server, SIGNAL(key_event(QString)), this, SLOT(onkeyEvent(QString)));
}
void MainWindow::mouse_control(QString x,QString y, QString type){
    QString cmd_str = "xdotool mousemove " + x + " " + y + " click " + type + " mousemove restore";
    char *str = new char[50];
    QByteArray ba = cmd_str.toLatin1();
    strcpy(str,ba.data());
    system(str);
}
void MainWindow::onkeyEvent(QString key){
    QString cmd = "xdotool key " + key;
    char *str = new char[50];
...
{
    QPixmap pix = QPixmap::grabWindow(0);
    QByteArray bytes;
    QBuffer buffer(&bytes);
    buffer.open(QIODevice::WriteOnly);
    pix.save(&buffer, "BMP");
    QByteArray medium = qCompress(bytes, -1);
    medium.insert(0,"SS");
    medium.append("End");
    m_server->onSendMessage(medium);
}
