    ...
    , ui(new Ui::MainWindow)
{
    ...
    flg = false;
    pic_h = pic_w = 0;
    ui->key_con->installEventFilter(this);
    //ui->server_screen->installEventFilter(this);
}
...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    int a = event->button();
    QString pos = "x:" + QString::number(pic_w*event->pos().x() / 570) + "y:" +
            QString::number((pic_h* (event->pos().y()-234) / 276)) + "type:"+QString::number(a) + "!";
    QByteArray m_pos;
    //m_pos.clear();
    m_pos.append(pos);
    socket->write(m_pos);
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
  {
      if (event->type() == QEvent::KeyPress) {
          ...
          QByteArray send_key;
          send_key.append("k:").append(keyEvent->text());
          socket->write(send_key);
          qDebug() << keyEvent->key();
          qDebug() << keyEvent->text();
          //keyEvent->
          ui->key_con->setText((QString) keyEvent->key());
          return true;
      }
    ...
    QByteArray array = socket->readAll();
        ...
        if(flg){
            if(array.contains("End")){
                ...
                QPixmap p;
                p.loadFromData(buffer,"BMP");
                pic_w = p.width();
                pic_h = p.height();
                ui->server_screen->setPixmap(p.scaled(570,309));
                results.clear();
                flg = false;
                return;
            }
