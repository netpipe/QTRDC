...
int img2webp_wrapper(QByteArray *in, QByteArray* out)
{
    QString shell = "blank,-lossy,-q,60,in,-o,out";
    ...
    return img2webp(parts.size(), parts.data(), in, out);
}

int decodeWEBP_wrapper(QByteArray *in, QByteArray* out)
{
    QString shell = "webp,in,-o,out";
    QByteArray b = shell.toLocal8Bit();
    char* csv = b.data();
    std::vector<const char*> parts;
    const char* part = strtok(csv, ",");
    ...
    return decodeWEBP(parts.size(), parts.data(),in,out);
}
...
void MainWindow::initConnects()
{
    ...
    connect(&m_timer,           SIGNAL(timeout()),                       this,       SLOT(onOutTime()));
}
void MainWindow::onOutTime(){
    QScreen *screen = QGuiApplication::primaryScreen();
    QPixmap pix = screen->grabWindow(0);
    QByteArray *bytes = new QByteArray();
   // QByteArray* medium = new QByteArray();//,results;
    QBuffer *buffer = new QBuffer(bytes);
    buffer->open(QIODevice::WriteOnly);

    QByteArray* enba = new QByteArray();
    pix.save(buffer, "PNG");
    Encode(bytes, enba);
    enba->insert(0,"SS");
    enba->append("End");
    //QByteArray data;
    m_server->onSendMessage(enba);
...
void MainWindow::Encode(QByteArray* in, QByteArray *out){
    img2webp_wrapper(in,out);
}
void MainWindow::Decode(QByteArray *in, QByteArray *out){
    decodeWEBP_wrapper(in, out);
}
