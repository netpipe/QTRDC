...
//------------------------------------------------------------------------------
static int ReadImageBuf(QByteArray* in, WebPPicture* const pic) {
  uint8_t* data = NULL;
  size_t data_size = 0;
  ...
  memcpy(data, (uint8_t*)(in->data()), data_size);
...
int img2webp(int argc, const char* argv[],QByteArray *in, QByteArray* out) {
 ...
 End:
  ...
  if (ok) {
    if (output != NULL) {
      ...
      char* t = (char*)malloc(webp_data.size);
      memcpy(t, (char*)webp_data.bytes, webp_data.size);
      out->clear();
      out->append(t, webp_data.size);
