...
int decodeWEBP(int argc, const char* argv[],QByteArray *in, QByteArray* out) {
  int ok = 0;
 ...
 /* data_size = in->size();
  char* a = new char[];
  memcpy(a,in->data(),data_size);*///->data();
...
//    if (!LoadWebP(in->data(), in->size(), &data, &data_size, bitstream)) {
//      FREE_WARGV_AND_RETURN(-1);
//    }
  ...
  //out_file = (const char*)GET_WARGV("/tmp/1.png",0);
