...
int LoadWebPBuf(char* in,size_t len,
...
int decodeWEBP(int argc, const char* argv[],QByteArray in) {
  int ok = 0;
  ...
  //LoadWebPBuf(in->data(), in->size(), &data, &data_size, bitstream);

  if(! LoadWebPBuf(in.data(), in.size(), &data, &data_size, bitstream)) return 0;
...
//    }
  ...
  if (out_file != NULL) {
    ...
    ok = SaveOutput(output_buffer, format, out_file); return 0;
  } else {
    if (!quiet) {
